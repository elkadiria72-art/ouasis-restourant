      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-700">
          <thead className="bg-slate-900/80">
            <tr>
              <th className="px-5 py-4 text-right text-sm font-semibold text-slate-300">الاسم</th>
              <th className="px-5 py-4 text-right text-sm font-semibold text-slate-300">الدور</th>
              <th className="px-5 py-4 text-right text-sm font-semibold text-slate-300">الحالة</th>
              <th className="px-5 py-4 text-right text-sm font-semibold text-slate-300">الإجراءات</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-700">
            {staff.map((member) => (
              <tr key={member.id} className="transition-colors hover:bg-slate-750/60">
                ...
                <td className="px-5 py-4">
                  <select
                    value={member.role}
                    onChange={(event) =>
                      onRoleChange(member.id, event.target.value as StaffMember['role'])
                    }
                ...
                <td className="px-5 py-4">
                  ...
                  >
                    ...
                    {member.status === 'online'
                      ? ar.staffStatus.online
                      : ar.staffStatus.offline}
                  </span>
                </td>
                ...
                <td className="px-5 py-4">
                  <button
                    ...
                    onClick={() =>
                      onToggleStatus(
                        member.id,
                        member.status === 'online' ? 'offline' : 'online'
                      )
                    }
                  ...
                  >
                    <Power size={14} />
                    {member.status === 'online' ? 'تعطيل' : 'إعادة تفعيل'}
                  </button>
                </td>
              </tr>
            ))}
